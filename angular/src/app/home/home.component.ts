...
import { Home } from '../shared/services/home.service';
...
export class HomeComponent implements OnInit {
    ...
    chatmessage: any;
    chatform: FormGroup;
    darkScrollbarOptions = { theme: 'dark' };
    submitted = false;
    ...
    ngOnInit() {
        ...
        this.homeservice.getBotman(this.start_contversion,
            res => {
                let data = res.json();
                var i = 1;
    ...
    scrollEvent() {
        this.mScrollbarService.initScrollbar('#light-card', this.darkScrollbarOptions);
    ...
    onSubmit() {
        ...
        if (!this.chatform.valid){
            ...
            this.homeservice.getBotman(this.chatform.value,
                res => {
                    let data = res.json();
                    var i = 1;
                    for (let chatdata of data.messages) {
